...
const AirportVertex Graph::InvalidVertex = "_CS225INVALIDVERTEX";
     ...
     random(Random(seed)) 
{
    ...
    AirportVertex cur = vertices[0];
    for (size_t i = 0; i < vertices.size() - 1; ++i)
    {
        AirportVertex next = vertices[i + 1];
        insertEdge(cur, next);
        if (weighted) 
        {
...
vector<AirportVertex> Graph::getAdjacent(AirportVertex source) const 
{
    auto lookup = adjacency_list.find(source);

    if(lookup == adjacency_list.end())
        return vector<AirportVertex>();

    else
    {
        vector<AirportVertex> vertex_list;
        unordered_map <AirportVertex, Edge> & map = adjacency_list[source];
...
AirportVertex Graph::getStartingVertex() const
{
    return adjacency_list.begin()->first;
}

vector<AirportVertex> Graph::getVertices() const
{
    vector<AirportVertex> ret;
...
Edge Graph::getEdge(AirportVertex source , AirportVertex destination) const
{
...
vector<Edge> Graph::getEdges() const
{
    ...
    vector<Edge> ret;
    set<pair<AirportVertex, AirportVertex>> seen;

    for (auto it = adjacency_list.begin(); it != adjacency_list.end(); it++)
    {
        AirportVertex source = it->first;
        for (auto its = adjacency_list[source].begin(); its != adjacency_list[source].end(); its++)
        {
            AirportVertex destination = its->first;
...
bool Graph::vertexExists(AirportVertex v) const
{
    return assertVertexExists(v, "");
}

bool Graph::edgeExists(AirportVertex source, AirportVertex destination) const
{
    return assertEdgeExists(source, destination, "");
}

Edge Graph::setEdgeLabel(AirportVertex source, AirportVertex destination, string label)
{
    if (assertEdgeExists(source, destination, __func__) == false)
        return InvalidEdge;
...
string Graph::getEdgeLabel(AirportVertex source, AirportVertex destination) const
{
    if(assertEdgeExists(source, destination, __func__) == false)
        return InvalidLabel;
...
double Graph::getEdgeWeight(AirportVertex source, AirportVertex destination) const
{
    if (!weighted)
...
void Graph::insertVertex(AirportVertex v)
{
    // will overwrite if old stuff was there
    removeVertex(v);
    // make it empty again
    adjacency_list[v] = unordered_map<AirportVertex, Edge>();
}


AirportVertex Graph::removeVertex(AirportVertex v)
{
...
bool Graph::insertEdge(AirportVertex source, AirportVertex destination)
{
    ...
    if(adjacency_list.find(source)==adjacency_list.end())
    {
        adjacency_list[source] = unordered_map<AirportVertex, Edge>();
    }
    ...
    if(!directed)
    {
        ...
        {
            adjacency_list[destination] = unordered_map<AirportVertex, Edge>();
        }
...
Edge Graph::removeEdge(AirportVertex source, AirportVertex destination)
{
    if(assertEdgeExists(source, destination, __func__) == false)
        return InvalidEdge;
    Edge e = adjacency_list[source][destination];
...
Edge Graph::setEdgeWeight(AirportVertex source, AirportVertex destination, double weight)
{
    if (assertEdgeExists(source, destination, __func__) == false)
        return InvalidEdge;
    Edge e = adjacency_list[source][destination];
...
bool Graph::assertVertexExists(AirportVertex v, string functionName) const
{
    if (adjacency_list.find(v) == adjacency_list.end())
    {
        if (functionName != "")
...
bool Graph::assertEdgeExists(AirportVertex source, AirportVertex destination, string functionName) const
{
...
void Graph::savePNG(string title) const
{
    ...
    vector<AirportVertex> allv = getVertices();
    //lambda expression
    sort(allv.begin(), allv.end(), [](const AirportVertex& lhs, const AirportVertex& rhs) {
