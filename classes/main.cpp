   ...
   //will repeat until proper responses were given
    while (!proper_response) {
       //storing user input into user variables
        ...
        proper_response = p.isProperResponse(userSource, userDestination);
        if (proper_response) {
            std::cout <<"\n" <<std::endl;
            std::cout << "Calculating route... \n" <<std::endl;
            std::cout << "Currently ensuring you have the best travel experience because YOU deserve it! \n" <<std::endl;

            f->storeUserInput(userSource, userDestination);
            AirportVertex s(userSource);
            AirportVertex z(userDestination);
            vector<string> path = d.dijkstra(f->world, s, z);
